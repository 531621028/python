...
def get_content(url):
    ...
    for li in liTags:
        ...
        try:
            ...
            # .text属性为指定标签(Tag)中所有text的内容(包含子孙节点)
            ...
            comment['replyNum'] = li.find(
                'span', attrs={'class':
                               'threadlist_rep_num center_text'}).text.strip()
            comments.append(comment)
        except Exception as e:
            print('解析出问题了', comment['title'], e)
    ...
    with open('TTBT.txt', 'a+', encoding='utf-8') as f:
        for comment in comments:
            f.write('标题： {} \t 链接：{} \t 发帖人：{} \t 发帖时间：{} \t 回复数量： {} \n'.
                    format(comment['title'], comment['link'], comment['name'],
                           comment['time'], comment['replyNum']))
        print('当前页面爬取完成')
